...
task :email_return_reminder => :environment do
  ...
  @requests = BorrowRequest.where('(return_status = 1 or return_status = 4) AND (return_date <= ?)', Date.today + 2.day)
